...
public class EndpointsPlaygroundCmdSnapshotTest extends BaseEndpointCmdSnapshotTest {
    @Override
    @TestForAllOutputs
    public void endpoint_found(OutputType outputType) {
        verifyRun("db endpoints playground ${DatabaseId}", outputType, opts);
    }

    @Override
    @TestForDifferentOutputs
    public void endpoint_found_for_region(OutputType outputType) {
        verifyRun("db endpoints playground ${DatabaseId} -r ap-south-1", outputType, opts);
    }
}
