...
public abstract class BaseRegionsCmdSnapshotTest extends BaseCmdSnapshotTest {
    private static final SortedMap<CloudProvider, ? extends SortedMap<String, RegionInfo>> FOUND_REGIONS = new TreeMap<>() {{
    ...
    protected BaseRegionsCmdSnapshotTest(String... subcommand) {
    ...
    public void regions_found(OutputType outputType) {
        verifyRun("db " + String.join(" ", subcommand), outputType, mkOpts(FOUND_REGIONS));
    }

    @TestForDifferentOutputs
    public void regions_found_with_name_filter(OutputType outputType) {
        verifyRun("db " + String.join(" ", subcommand) + " --filter east", outputType, mkOpts(FOUND_REGIONS));
    }

    @TestForDifferentOutputs
    public void regions_found_with_cloud_filter(OutputType outputType) {
        verifyRun("db " + String.join(" ", subcommand) + " -c gcp,aws", outputType, mkOpts(FOUND_REGIONS));
    }

    @TestForDifferentOutputs
    public void regions_found_with_zone_filter(OutputType outputType) {
        verifyRun("db " + String.join(" ", subcommand) + " --zone Europe", outputType, mkOpts(FOUND_REGIONS));
    }

    @TestForDifferentOutputs
    public void no_regions_found(OutputType outputType) {
        verifyRun("db " + String.join(" ", subcommand), outputType, mkOpts(new TreeMap<>()));
    }
}
