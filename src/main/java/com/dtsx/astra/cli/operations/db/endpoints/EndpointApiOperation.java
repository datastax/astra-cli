...
import static com.dtsx.astra.cli.operations.db.endpoints.EndpointApiOperation.*;

@RequiredArgsConstructor
public class EndpointApiOperation implements Operation<String> {
    private final DbGateway dbGateway;
    private final EndpointApiRequest request;

    public record EndpointApiRequest(
        DbRef dbRef,
    ...
    public String execute() {
        ...
        return (db.getInfo().getDbType() == null)
            ? ApiLocator.getApiRestEndpoint(request.env, db.getId(), effectiveRegion.unwrap())
            : ApiLocator.getApiEndpoint(request.env, db.getId(), effectiveRegion.unwrap());
    }
}