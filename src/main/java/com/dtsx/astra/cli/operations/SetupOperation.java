...
public class SetupOperation implements Operation<SetupResult> {
    ...
    public record SetupRequest(
        ...
        BiFunction<SetupActionSelector, Path, SetupAction> promptForNextActionIfExistingUser,
    ...
    public SetupResult execute() {
        ...
        return resolveAction(configFile).execute(configFile, request);
    }

    private SetupAction resolveAction(Path configFile) {
        if (ifConfigExistsAnd(c -> !c.profiles().isEmpty())) {
            return request.promptForNextActionIfExistingUser.apply(new SetupActionSelector(), configFile);
        } else {
            request.assertShouldSetup.accept(configFile);
            return new CreateProfileSetupAction();
        }
    }
    ...
    public interface SetupAction {
        SetupResult execute(Path configFile, SetupRequest request);
    }

    public class SetupActionSelector {
        public SetupAction createProfile() {
            return new CreateProfileSetupAction();
        }

        public SetupAction explainAutocomplete() {
            return (_, _) -> new ExplainAutocomplete();
        }
    ...
    public class CreateProfileSetupAction implements SetupAction {
        @Override
        public SetupResult execute(Path configFile, SetupRequest request) {
