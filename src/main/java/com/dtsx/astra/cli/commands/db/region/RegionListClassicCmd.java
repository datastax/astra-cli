...
@Example(
    ...
    command = "${cli.name} db list-regions-classic"
)
@Example(
    comment = "Filter by cloud provider",
    command = "${cli.name} db list-regions-classic --cloud AWS,GCP"
)
@Example(
    comment = "Filter by partial region name",
    command = "${cli.name} db list-regions-classic --filter us-,ca-"
)
@Example(
    comment = "Filter by zone",
    command = "${cli.name} db list-regions-classic --zone Europe"
)
